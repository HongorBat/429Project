...
public class VendorInventoryItemType extends EntityBase {
	...
	/**
	 * Constructor gets the inventory item type from the constructor
	 * @param iitn
	...
	public VendorInventoryItemType(String id) throws InvalidPrimaryKeyException {
		...
		if (allDataRetrieved != null) {
			...
			if (size != 1) {
				throw new InvalidPrimaryKeyException("Multiple VendorInventoryItemTypes matching id : " + id + " found.");
			} else {
			...
			throw new InvalidPrimaryKeyException("No VendorInventoryItemType matching id : " + id + " found.");
		}
	}
	...
	public void update() {
		try {
			if (persistentState.getProperty("Id") != null) {
				...
				updateStatusMessage = "VendorInventoryItemTypes matching  : " + persistentState.getProperty("Id") + " updated successfully in database!";
			} else {
				Integer id = insertAutoIncrementalPersistentState(mySchema, persistentState);
				persistentState.setProperty("Id", "" + id.intValue());
				updateStatusMessage = "VendorInventoryItemTypes matching : " +  persistentState.getProperty("Id") + "installed successfully in database!";
			}
		} catch (SQLException ex) {
			updateStatusMessage = "Error in installing VendorInventoryItemType data in database!";
		}
	}
}